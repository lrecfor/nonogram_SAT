    ...
    def solve(self, with_=False):
        ...
        for i, c in enumerate(self.rows):
            ...
            for config in self.convert(self.width, c):
                x = list(range(i * self.width + 1, (i+1) * self.width + 1))
        ...
        for i, c in enumerate(self.cols):
            ...
            for config in self.convert(self.height, c):
                x = [i % self.width + self.width * j + 1 for j in range(self.width)]
                for v in [a * b for a, b in zip(x, config)]:
                    instance.add_clause([-compteur, v])
                    # print([-compteur, v], end="")
                lines.append(compteur)
        ...
        if with_:
            ...
            for i in range(0, len(result)):
                if result[i] == Status.black:
                    instance.add_clause([i + 1])
                elif result[i] == Status.cross:
                    instance.add_clause([-(i + 1)])

        print("CONVERT TIME: ", time.time() - a)
        a = time.time()
